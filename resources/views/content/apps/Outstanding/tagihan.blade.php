...
.card-border-shadow-danger::before {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
...
.card-border-shadow-danger::before {
  background: linear-gradient(180deg, #ff4757 0%, #ee1d3a 100%);
}
...
.btn-danger {
  background: linear-gradient(135deg, #ff4757 0%, #ee1d3a 100%);
  box-shadow: 0 2px 8px rgba(255, 71, 87, 0.3);
}

.btn-danger:hover {
  background: linear-gradient(135deg, #ee1d3a 0%, #d70026 100%);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // ========================================
    // FILTER OUTSTANDING BUTTON
    // ========================================
    let isOutstandingFilter = false;
    
    $('#btnFilterOutstanding').on('click', function() {
        isOutstandingFilter = !isOutstandingFilter;
        
        if (isOutstandingFilter) {
            // Filter hanya tagihan belum bayar
            dtUserTable.column(4).search('Belum Bayar').draw();
            $(this).removeClass('btn-outline-danger').addClass('btn-danger');
            $(this).html('<i class="ri-error-warning-line me-1"></i>Outstanding Aktif');
        } else {
            // Reset filter
            dtUserTable.column(4).search('').draw();
            $(this).removeClass('btn-danger').addClass('btn-outline-danger');
            $(this).html('<i class="ri-error-warning-line me-1"></i>Lihat Outstanding');
        }
    });
    ...
    <div class="card-header">
      <div class="row align-items-center">
        ...
        <div class="col-md-6 text-md-end mt-3 mt-md-0">
                   <button type="button" class="btn btn-outline-danger me-2" data-bs-toggle="modal" data-bs-target="#modalTambahTagihan">
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <form action="{{ route('tagihan.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-header bg-primary">
          <h5 class="modal-title text-white fw-bold">
            <i class="ri-add-circle-line me-2"></i>Buat outstanding baru
          </h5>
          <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
        </div>
